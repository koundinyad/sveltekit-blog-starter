...
<script>
	...
	// Array of colors
	const colors = ['#CAC444', '#ED7BA1', '#ED4005', '#91CBDA', '#C1D200'];
	...
	const projectImages = import.meta.glob('/static/images/**/*.{jpg,jpeg,png,gif,webp,webm}', { eager: true });
	...
	function getImages(slug) {
		...
		return Promise.all(Object.keys(projectImages).map(loadImageData))
	...
	(async () => {
		...
		for (const p of projects) {
			try {
				...
				$allProjects = [{metadata: p.metadata, default: p.default, images: filteredImages}, ...$allProjects];
			} catch (error) {
				console.error('Error loading images:', error);
			}
		}
		...
		{#if project.metadata.featured === "yes"}
			...
			<div class="md:col-start-4 md:col-span-6 col-start-1 col-span-6 ">
				<div class="flex flex-row justify-between items-end">
					<p class="text-xl mb-0">{project.metadata.title}<sup class="text-gray text-sm">{project.metadata.type} ({project.metadata.year})</sup></p>
					{#if project.metadata.link}
						<a href={project.metadata.link} target="_blank" class="text-sm px-2 py-1 rounded text-white" style="background-color: {getRandomColor()};">Visit ↗</a>
				...
				<EmblaCarousel>
					{#if project.images.length > 0}
						{#each project.images as { src, alt }}
							<div class="embla__slide flex-none w-full h-[400px]">
								<img class="object-cover w-full h-[400px]" src={src.replace(/^\/static/, '')} {alt} />
							</div>
						{/each}
					{:else}
			...
			{#each $allProjects as project}
				...
				<div class="flex-1">
					<div class="flex flex-col">
						<!-- project title -->
						<p class="mb-2">{project.metadata.title}<sup class="text-gray hidden md:inline-block">{project.metadata.type} ({project.metadata.year})</sup></p>
						<!-- project details -->
						<p class="block md:hidden text-gray mb-0">{project.metadata.type} ({project.metadata.year})</p>
						<!-- project images -->
						<div class="my-2">
							<EmblaCarousel>
								{#if project.images.length > 0}
